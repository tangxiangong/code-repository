function [t, invS, varargout] = inv_stable_subordinator(alpha, T, tau)
    ...
    [t_temp, s_temp] = stable_subordinator(alpha, T, tau);
    temp = 1;
    while s_temp(end) < T
        [t_temp, s_temp] = stable_subordinator(alpha, 2^temp*T, tau);
        temp = temp + 1;
    end
    index = s_temp <= T;
